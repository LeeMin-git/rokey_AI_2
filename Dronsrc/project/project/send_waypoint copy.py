...
class WaypointFollower(Node):
    ...
    def create_waypoints(self, num):
        ...
        if num == 1:#순회 포인트
            ...
            waypoint5.header.stamp= self.get_clock().now().to_msg()
            ...
            waypoint6.header.stamp= self.get_clock().now().to_msg()
    ...
    def callback_center_point(self,data):
        if data.centerx != 0.0:
            if self.detect_object == False:
                ...
                self.get_logger().info('current destination cancel....')
                self.cancel_goal()
            else:
            ...
            if self.detect_object == True:
                self.get_logger().info('not found object... :( )')
                self.center_p.linear.x=0.0
    ...
    def callback_nav_state(self,data):
        ...
        if self.goal_seq == '2':
            self.send_goal(2)
            self.get_logger().info('waypoint_sequence: out home')
        elif self.goal_seq == '3':
            self.send_goal(3)
            self.home_out = False ## 홈으로 들어가니까 home_out 후에 바로 순회시키기 위해
            self.get_logger().info('waypoint_sequence: go home')
        elif self.goal_seq == '1':
            self.send_goal(1)
            self.get_logger().info('waypoint_sequence: move around')
    ...
    def pose_init(self):
        ...
        self.get_logger().info('succeed pose init!!!')
