...
class CompressedImageSubscriber(Node):
    def __init__(self):
        ...
        self.model = YOLO("/home/rokey6/rokeyI_C2_ws/src/object_detection/resource/best_ver2.pt")
        ...
        self.cmd_timer = self.create_timer(0.5, self.callback_cmd_time)
        self.center_x = 0.0
        self.center_y = 0.0
        self.center_p = Twist()
    ...
    def callback_cmd_time(self):
        if self.center_x >= -30 and self.center_x<=30:
            self.center_p.angular.z=0.0
            
            if self.center_y >= -10 and self.center_y<=10:
                self.center_p.linear.x=0.0
            else:
                self.center_p.linear.x=-self.center_y/500
                self.center_p.angular.z=0.0
                self.pub_cmd_vel.publish(self.center_p)
                print(self.center_y)
        else:
            self.center_p.linear.x=0.0
            self.center_p.angular.z=-self.center_x/500
        self.pub_cmd_vel.publish(self.center_p)
